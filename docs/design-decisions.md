...
**决策**：使用 tmuxp 声明式 YAML 管理 tmux session 布局，每个 agent/WeeChat 使用独立 window（非 pane）。

**原因**：原先所有组件挤在一个 window 的 pane 中，空间不足。使用 `sleep 3 && tmux send-keys Enter` 自动确认 Claude 启动提示不可靠（时序依赖）。改为 `capture-pane` 轮询检测提示 + `SessionStart` hook 写 ready marker 的两阶段就绪检测。tmuxp 提供声明式 session 定义，支持 shutdown 后恢复布局。
