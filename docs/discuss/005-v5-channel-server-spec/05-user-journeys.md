...
3. 启动 channel-server 独立进程              ← fixture 自动
   → uv run zchat-channel（IRC bot + Bridge API :9999 + engine）
4. zchat agent create fast-agent             ← fixture 自动
   → Zellij tab → Claude Code → agent_mcp.py（轻量 MCP + IRC）
   → 等待 .ready marker（最多 60s）
5. zchat agent create deep-agent             ← fixture 自动（同上）
6. feishu_bridge 启动                        ← fixture 自动（连接 ws://localhost:9999）
7. 飞书测试群中发消息                         ← FeishuTestClient 自动
8. Claude 真实响应 → agent_mcp → IRC → channel-server → Bridge → 飞书
9. 验证消息路由和 visibility                  ← assert_message_appears/absent
```

**关键点**: channel-server 是**独立进程**（IRC bot + Bridge API + engine），先于所有 agent 启动。每个 agent 运行轻量 `agent_mcp.py`（MCP stdio + IRC 连接），通过 IRC 与 channel-server 交互。

**CI 降级**: CI 环境可只启动 channel-server + agent_mcp（无 Claude Code），通过 IRC 直接发消息验证协议行为。
