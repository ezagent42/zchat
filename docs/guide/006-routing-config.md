...
credential_file = "credentials/customer.json"  # ✅ 必填，文件内含 {"app_id":"...","app_secret":"..."}
...
| 字段 | 类型 | 必填 | 谁读 | 含义 |
|---|---|---|---|---|
| `credential_file` | str | ✅ | bridge | 相对 `~/.zchat/projects/<name>/` 的凭证文件路径；文件内容 schema **由 bridge 自己定**（feishu_bridge 的实际内容是 `{"app_id": "...", "app_secret": "..."}`，其它平台自己定义）。V7 起 `app_id` 仅从此文件读取 |
...
```bash
# 推荐: 先写 credentials/<name>.json {"app_id":"X","app_secret":"Y"}，再注册（自动检测）
...
# ── 3 个飞书 bot ────────────────────────────────────────
[bots.customer]
credential_file = "credentials/customer.json"   # {"app_id":"cli_a954...","app_secret":"..."}
