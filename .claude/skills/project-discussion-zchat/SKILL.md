...
| doctor_update | `zchat/cli/doctor.py` + `update.py` + `audit_cmd.py` | 环境诊断 + 自动升级 + audit 只读 CLI (V7: 读 `<project>/plugins/audit/state.json`；admin-agent 通过 `run_zchat_cli` 调用) | `bash scripts/test-doctor_update.sh` | 43 passed | `zchat doctor`, `zchat audit status/report`, 自升级 |
